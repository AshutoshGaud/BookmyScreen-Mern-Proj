...
export const movieSchema = z.object({
  ...
  releaseDate: z.string().transform((val) => new Date(val)), // ✅ fixed syntax
  ...
  rating: z.number().min(0).max(10), // ✅ fixed typo
  votes: z.number().min(0),
  format: z.array(z.string()).default(["2D"]),
});

export type MovieInput = z.infer<typeof movieSchema>; // ✅ fixed name
